        ...
        if relLocn[i][1][1] in pseudoType:
            ...
            if len(temp) == 2:
                if not(temp[0].isnumeric()):
                    t = valueCalculator.valFromSymbol(temp[0],symTab)
                    if t == -1:
                        t = valueCalculator.valFromLiteral(temp[0],litTab)
                    temp[0] = t
                if not(temp[1].isnumeric()):
                    t = valueCalculator.valFromSymbol(temp[1],symTab)
                    if t == -1:
                        t = valueCalculator.valFromLiteral(temp[1],litTab)
                    temp[1] = t
            else:
                if not(temp[0].isnumeric()):
            ...
            if len(temp) == 2:
                ...
                if temp[1].find(')') != -1 and temp[1][0] != '=':
                    index = valueCalculator.indexExtractor(temp[1],bTab,symTab)
                    if temp[1][0] != '=':
                ...
                temp[1] = valueCalculator.calcAddress(temp[1],index,basePointer,bTab)
                arg = str(temp[0]) + "," + str(temp[1])                
                machTab.append([relLocn[i][0],[cmd , arg]])  
